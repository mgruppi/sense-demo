...
def generate_example(new_example_id, example, all_alignment_configs, EMBEDDING_PREFIX, sent_1_path, sent_2_path):
    ...
    wv1_path = EMBEDDING_PREFIX + example["embedding_a_id"] + ".txt"
    wv2_path = EMBEDDING_PREFIX + example["embedding_b_id"] + ".txt"
    wv1 = WordVectors.from_file(wv1_path)
    wv2 = WordVectors.from_file(wv2_path)
    return Example(new_example_id, wv1, wv2, config_objs, sent_1_path, sent_2_path)

def main(examples_config_path, APPLICATION_CONSTANTS_FILENAME):
    ...
    with open(examples_config_path) as examples_config_file:
    ...
    with open(examples_config_path, "w") as config_file:
